...
```text
src/
  assets/       reserved source-asset directory
...
Keep browser-served application media in `public/assets`; authored content references those files through the shared asset helper. Do not add generated asset metadata sidecars under `src/assets`. Put other files that must be available directly from the browser, such as robots metadata, in `public`; the Pages export copies the resulting public output into the generated root. Put repository automation and export behavior in `scripts`; put container lifecycle and validation in `.devcontainer`; put cross-cutting explanations in `docs`.
